...
type CaseFormErrors = Partial<Record<"approvedAmount" | "tenderName" | "tenderNo" | "tmRemarks", string>>;
type DelayFormErrors = Partial<Record<"delayExternalDays" | "delayReason", string>>;
...
export function UpdateCasePanel({ caseId }: UpdateCasePanelProps) {
  ...
  const [milestones, setMilestones] = useState<MilestoneFormState>(emptyMilestones);
  ...
  useEffect(() => {
    ...
    setMilestones({
      ...
      commercialEvaluationDate: milestoneString(kase.milestones.commercialEvaluationDate),
      ...
      technicalEvaluationDate: milestoneString(kase.milestones.technicalEvaluationDate),
    });
  ...
  const caseErrors = useMemo(
    ...
    [approvedAmount, milestones, tenderName, tenderNo, tmRemarks],
  );
  ...
  const visibleDelayErrors: DelayFormErrors = showDelayErrors ? delayErrors : {};
  ...
  const caseChangedFields = useMemo(
    ...
    [approvedAmount, estimateBenchmark, detail.data, priorityCase, tenderName, tenderNo, tmRemarks],
  );
      ...
      updateCase(caseId as string, {
        financials:
          approvedAmount.trim() || estimateBenchmark.trim()
            ? {
                approvedAmount: approvedAmount.trim() ? parseMoneyInput(approvedAmount) : undefined,
                estimateBenchmark: estimateBenchmark.trim() ? parseMoneyInput(estimateBenchmark) : undefined,
              }
            : undefined,
        priorityCase,
  ...
  const updateMilestoneMutation = useMutation({
    mutationFn: () => updateMilestones(caseId as string, milestonePayload(milestones)),
    onSuccess: async () => {
      await invalidateCaseQueries(queryClient, caseId);
  ...
  function handleMilestoneSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (requiresApprovedAmount(milestones) && (!approvedAmount.trim() || caseErrors.approvedAmount)) return;
    ...
    <section className="state-panel case-detail-grid-wide case-edit-panel">
      ...
      <div className="update-workspace-grid">
        <form className="stack-form case-edit-card case-edit-card-primary" onSubmit={handleCaseSubmit}>
          ...
          <FormField error={visibleCaseErrors.tenderName ?? ""} label="Tender Name">
            <TextInput maxLength={500} onChange={(event) => setTenderName(event.target.value)} value={tenderName} />
          </FormField>
          <FormField error={visibleCaseErrors.tenderNo ?? ""} label="Tender No">
            <TextInput maxLength={200} onChange={(event) => setTenderNo(event.target.value)} value={tenderNo} />
          </FormField>
          ...
          <FormField error={visibleCaseErrors.tmRemarks ?? ""} label="TM Remarks">
            <textarea
          ...
          {updateCaseMutation.error ? <div className="form-error">{updateCaseMutation.error.message}</div> : null}
        ...
        <form className="stack-form case-edit-card" onSubmit={handleOwnerSubmit}>
          ...
          <FormField helperText="Completion target is governed by tender type rule and is locked after intake." label="Completion Target">
            <TextInput disabled type="date" value={detail.data?.tentativeCompletionDate ?? ""} />
          </FormField>
          {ownerMutation.error ? <div className="form-error">{ownerMutation.error.message}</div> : null}
          <Button disabled={!canReassignOwner || !ownerUserId || ownerMutation.isPending} type="submit">
            Save Owner
          </Button>
        </form>
        ...
        <form className="stack-form case-edit-card" onSubmit={handleDelaySubmit}>
          ...
          <FormField error={visibleDelayErrors.delayReason ?? ""} label="Delay Reason">
            <textarea
              className="text-input text-area"
              maxLength={5000}
          ...
          {delayMutation.error ? <div className="form-error">{delayMutation.error.message}</div> : null}
      ...
      <form className="stack-form case-edit-card case-edit-milestones" onSubmit={handleMilestoneSubmit}>
        ...
        <div className="milestone-form-grid">
          ...
          />
          <FormField error={visibleMilestoneErrors.biddersParticipated ?? ""} label="Bidders Participated">
            <TextInput
              min={0}
              onChange={(event) => setMilestones((value) => ({ ...value, biddersParticipated: event.target.value }))}
              type="number"
          ...
          />
          <FormField error={visibleMilestoneErrors.qualifiedBidders ?? ""} label="Qualified Bidders">
            <TextInput
              min={0}
              onChange={(event) => setMilestones((value) => ({ ...value, qualifiedBidders: event.target.value }))}
              type="number"
          ...
          <label className="checkbox-row">
            <input
              ...
              onChange={(event) => setMilestones((value) => ({ ...value, loiIssued: event.target.checked }))}
              type="checkbox"
            />
            LOI Issued
          </label>
...
function buildCaseChangedFields(
  ...
  if (value.tmRemarks !== (kase.tmRemarks ?? "")) fields.push("TM Remarks");
  ...
  if (
    ...
    parseMoneyInput(value.approvedAmount) !== (kase.financials.approvedAmount ?? null)
  ) {
  ...
  if (
    ...
    parseMoneyInput(value.estimateBenchmark) !== (kase.financials.estimateBenchmark ?? null)
  ) {
...
function buildDelayChangedFields(
  ...
  if (!kase) return [];
  const fields: string[] = [];
  if (value.delayExternalDays !== numberString(kase.delay.delayExternalDays)) fields.push("External Delay Days");
  if (value.delayReason !== (kase.delay.delayReason ?? "")) fields.push("Delay Reason");
  return fields;
}

function buildMilestoneChangedFields(kase: CaseDetail | undefined, value: MilestoneFormState) {
  if (!kase) return [];
  const fields: string[] = [];
  ...
  if (value.biddersParticipated !== numberString(kase.milestones.biddersParticipated)) {
    fields.push("Bidders Participated");
  }
  if (value.qualifiedBidders !== numberString(kase.milestones.qualifiedBidders)) {
...
function validateCaseForm(input: {
  ...
  if (input.tmRemarks.length > 5000) {
    errors.tmRemarks = "TM remarks cannot exceed 5000 characters.";
  }
  if (input.approvedAmount.trim() && !isMoneyInput(input.approvedAmount)) {
    errors.approvedAmount = "Approved amount must be a valid amount greater than or equal to 0.";
  }
  if (requiresApprovedAmount(input.milestones) && !input.approvedAmount.trim()) {
    errors.approvedAmount = "Approved amount is required before NFA approval, LOI, or RC/PO award.";
  }
  return errors;
}

function validateDelayForm(input: { delayExternalDays: string; delayReason: string }): DelayFormErrors {
  const errors: DelayFormErrors = {};
  if (input.delayExternalDays && !isNonNegativeInteger(input.delayExternalDays)) {
    errors.delayExternalDays = "External delay days must be a whole number greater than or equal to 0.";
  }
...
function validateMilestones(value: MilestoneFormState): MilestoneErrors {
  ...
  requireDateOrder(errors, value, "nfaSubmissionDate", "nfaApprovalDate", "NFA Approval cannot be before NFA Submission.");
  requireDateOrder(errors, value, "nfaApprovalDate", "loiIssuedDate", "LOI Issued Date cannot be before NFA Approval.");
  requireDateOrder(errors, value, "nfaApprovalDate", "rcPoAwardDate", "RC/PO Award cannot be before NFA Approval.");
  requireDateOrder(errors, value, "loiIssuedDate", "rcPoAwardDate", "RC/PO Award cannot be before LOI Issued Date.");
  requireDateOrder(errors, value, "rcPoAwardDate", "rcPoValidity", "RC/PO Validity cannot be before RC/PO Award.");

  if (value.loiIssued && !value.loiIssuedDate) {
    errors.loiIssuedDate = "LOI issued date is required when LOI is marked issued.";
  }

  validateNonNegativeInteger(errors, value.biddersParticipated, "biddersParticipated", "Bidders participated");
  validateNonNegativeInteger(errors, value.qualifiedBidders, "qualifiedBidders", "Qualified bidders");
  if (
    ...
    errors.qualifiedBidders = "Qualified bidders cannot exceed bidders participated.";
  }
...
function requiresApprovedAmount(value: MilestoneFormState) {
  return Boolean(value.nfaApprovalDate || value.loiIssued || value.loiIssuedDate || value.rcPoAwardDate);
}
...
function milestonePayload(value: MilestoneFormState) {
  return {
    ...
    biddersParticipated: value.biddersParticipated ? Number(value.biddersParticipated) : null,
    ...
    qualifiedBidders: value.qualifiedBidders ? Number(value.qualifiedBidders) : null,
